...
public interface OrderRepository extends JpaRepository<Order,Integer> {
    ...
    @Query(value = "select customer.fullName,o.date,p.name from customer  inner join orders o on customer.id = o.customer_id inner join detail d on o.id = d.order_id inner join product p on d.product_id = p.id order by o.date desc limit 1",nativeQuery = true)
    List<?> getLastOrders();
    ...
    @Query(value = "select p.name,order_id,quantity ,o.date from orders o inner join detail d on o.id = d.order_id inner join product p on p.id = d.product_id where o.id=:id",nativeQuery = true)
    List<?>getDetailsByOrderId(@RequestParam("id")Integer id);
}
